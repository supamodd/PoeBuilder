...
public static class DefenceCalculator
{
    ...
    /// <summary>Applies only the bounded passive ES recharge window after the last damage event.
    /// It deliberately does not model recovery modifiers or other sources such as recoup/leech.
    /// Invalid time/delay inputs return null rather than an invented value.</summary>
    public static decimal? EnergyShieldAfterRechargeWindow(decimal maximumEnergyShield,
        decimal currentEnergyShield, decimal secondsSinceDamage, decimal rechargePerSecond,
        decimal rechargeDelaySeconds)
    {
        if (maximumEnergyShield < 0 || currentEnergyShield < 0 || secondsSinceDamage < 0 || rechargeDelaySeconds < 0)
            return null;
        decimal current = Math.Clamp(currentEnergyShield, 0, maximumEnergyShield);
        if (maximumEnergyShield == 0 || secondsSinceDamage <= rechargeDelaySeconds || rechargePerSecond <= 0)
            return current;
        return Math.Min(maximumEnergyShield,
            current + (secondsSinceDamage - rechargeDelaySeconds) * rechargePerSecond);
    }
}
