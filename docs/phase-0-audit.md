...
7. **EHP:** report a typed vector per damage type for one successful hit using the same-level default monster raw hit; physical/elemental use Life + ES, chaos uses Life + ES/2 under the current PoB2 default double-ES-damage rule. A separate expected physical-attack estimate includes monster hit chance, attack block and deflection; the pure scenario helper also accepts attack dodge. The summary still has no dodge source, spell suppression, recovery, enemy configuration, penetration/exposure or explicit bypass integration; do not interpret either result as a universal survivability scalar.
