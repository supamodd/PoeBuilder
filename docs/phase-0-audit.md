...
| P1-03 | spell-block source integration, suppression source integration and full-scenario EHP remain absent; verified dodge source ids now flow through the interpreter/summary when present, while the pinned catalog currently has no player source | summary and interpreter contracts; `DefenceCalculator.cs`; `EhpCalculator.cs` | defence matrix plus recovery/bypass/mitigation fixture |
...
7. **EHP:** report a typed vector per damage type for one successful hit using the same-level default monster raw hit; physical/elemental use Life + ES, chaos uses Life + ES/2 under the current PoB2 default double-ES-damage rule. The calculator also exposes explicit typed attack/spell scenario contracts; the default summary remains limited to the same-level physical attack because the catalog has no spell-hit scenario. Recovery, enemy configuration, penetration/exposure and explicit bypass integration remain excluded; do not interpret either result as a universal survivability scalar.
