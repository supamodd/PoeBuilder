...
| P1-02 | active skill reservation enumeration and life/mana/ES recovery state absent; explicit resolved reservation context now has bounded reserved/unreserved output, and ES recharge has a bounded rate/delay/window helper | `CharacterCalculator.cs`; `ResourceCalculator.cs`; `DefenceCalculator.cs`; `StatInterpreter.cs` | active reserving-skill/reserved-resource/recovery/interruption cases |
