...
- life/mana/ES conversion, overrides, more/less, life/mana recovery, leech и damage bypass отсутствуют; ES recharge rate/delay отображаются как bounded panel estimate, а pure helper может применить окно после последнего damage event без полноценного combat interruption state;
