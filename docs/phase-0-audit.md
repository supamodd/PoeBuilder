...
5. **Suppression (partial follow-up):** helper covers cap `100%`, base effect `50%` and partial/full multipliers; `EhpCalculator` now composes suppression with optional spell dodge for an explicit scenario, but current catalog has no player source and build-level spell damage/EHP integration still requires a PoB2 fixture (**VERIFY**).
6. **Dodge (contract only):** `DefenceCalculator.DodgeChance` applies the current reference cap `75%`; the pinned catalog has no player attack/spell dodge source, so no summary row or EHP integration is invented.
7. **EHP:** report a typed vector per damage type for one successful hit using the same-level default monster raw hit; physical/elemental use Life + ES, chaos uses Life + ES/2 under the current PoB2 default double-ES-damage rule. A separate expected physical-attack estimate includes monster hit chance, attack block and deflection. Spell suppression, recovery, enemy configuration, penetration/exposure and explicit bypass modifiers remain excluded; do not interpret either result as a universal survivability scalar.
